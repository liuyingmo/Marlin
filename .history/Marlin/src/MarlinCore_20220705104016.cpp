...
void setup() {
  ...
  extern void task_jumpto_updata(uint8_t id,int32_t ntick);
  task_init();
  task_get(task_jumpto_updata);
...
void loop() {
  do {
    ...
    if(RST_EXTERNAL==hal.get_reset_source())
       jumpToSystemBoot();
...
void task_jumpto_updata(uint8_t id,int32_t ntick)
{
   crBegin(id, ntick);
   crWait(id,5000);
   jumpToSystemBoot();
