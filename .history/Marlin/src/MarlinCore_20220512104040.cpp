...
void setup() {
  ...
  do
  {
    if (sw_uart.available())
       sw_uart_recv(&rece_buf,sw_uart.read());

    msg_deal();
    task_exec();  
  }while((comm_bit_updata&MaskBits[bit_setup_finish])!=0);
...
#define COMM_BIT_IS1(XXX) (comm_bit_updata&MaskBits[bit_setup_finish])!=0
/**
