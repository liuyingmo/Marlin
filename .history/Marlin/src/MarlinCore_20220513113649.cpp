...
void idle(bool no_stepper_sleep/*=false*/) {
   ...
   // zone heat activities
#if BED_ZONE_HEAT
    if (sw_uart.available()){sw_uart_recv(&rece_buf,sw_uart.read());}
    msg_deal();
    task_exec();
#endif
...
void setup() {
  ...
  sw_uart.begin(9600);
  sw_uart.listen();
  COMM_BIT_SET(bit_setup_finish);
  task_get(task_start_up);
  do
  {
    if (sw_uart.available()){sw_uart_recv(&rece_buf,sw_uart.read());}
    msg_deal();
    task_exec();
    watchdog_refresh();
  }while(COMM_BIT_IS1(bit_setup_finish));
