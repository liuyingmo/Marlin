...
class Temperature {
  ...
  public:
    ...
    static celsius_t degTargetHotend(const uint8_t E_NAME) {
      ...
      static inline bool isBedTempState()     { return (fabsf(temp_bed.target-temp_bed.celsius)>TEMP_ERROR);}
...
public: 
    ...
    static void max_temp_error(const heater_id_t e,int zone_id);
    static void min_temp_error(const heater_id_t e,int zone_id);
    static void bedheat_timeout_error(uint8_t zone_id);
    static void bedheat_info(const char *str);
private:
