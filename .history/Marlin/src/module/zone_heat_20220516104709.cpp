...
void zooe_state_reset(void)
{
  LOOP_L_N(i,SUM_TEMP_ZONE){
    comm_dat.zooe_state[i] =0;
  }
}

bool zooe_temp_ach_target(void)
{
  LOOP_L_N(i,SUM_TEMP_ZONE){
    if(par_config.used[i]){
      if(!comm_dat.zooe_state[i]) return false;
    }
  }
  return thermalManager.isBedTempAbs();
}
...
void msg_cmd_bt303(uint8_t *pmsg)
{
  COMM_BIT_CLS(bit_auto_pid);
}

void msg_cmd_bt304(uint8_t *pmsg)
{
  char *token;
  while(*pmsg==' ')pmsg++;
  token = strtok((char*)pmsg," ");
  if(token==NULL)return;
  pid_bit_updata =atoi(token);
  COMM_BIT_CLS(bit_pid_state);
}

void msg_cmd_d101(uint8_t *pmsg)
{
  const char *p[2];
  int limit_min,limit_max;
  if(spit_string((char*)pmsg," ",p,2)==2)
  {
    limit_min =atoi(p[0]);
    limit_max =atoi(p[1]);
    cmd_string(CMD_BD101,"");

    LOOP_L_N(i,SUM_TEMP_ZONE){
    
      if(limit_max &MaskBits[i]){
        thermalManager.max_temp_error(H_BED,i+1);
        return;
      }

      if(limit_min &MaskBits[i]){
        thermalManager.min_temp_error(H_BED,i+1);
        return;
      }
    }
  }
...
void msg_cmd_d102(uint8_t *pmsg)
{
  int heat_id;
  char*token = strtok((char*)pmsg," ");
  if(token==NULL)return;
  heat_id =atoi(token);
  cmd_string(CMD_BD102,"");
  thermalManager.bedheat_timeout_error(heat_id);
}

void msg_cmd_d103(uint8_t *pmsg)
{
   to_lower(pmsg);
   SERIAL_ECHOLNPGM_P((const char*)pmsg);
}

void msg_cmd_bt107(uint8_t *pmsg)
{
  COMM_BIT_CLS(bit_set_pid);
}

void msg_cmd_bt102(uint8_t *pmsg)
{
  comm_bit_updata&=~MaskBits[bit_start_heating];
  COMM_BIT_CLS(bit_start_heating);
}
...
uint8_t get_heat_pwm(void)
{
  ...
  LOOP_L_N(i,SUM_TEMP_ZONE){
    if(par_config.used[i]){
      pwm+=comm_dat.zone_pwm[i];
      zone_cnt++;
    }
  }
  return (zone_cnt==0?0:pwm/zone_cnt);
}
...
celsius_float_t get_heat_temp(void)
{
  ...
  LOOP_L_N(i,SUM_TEMP_ZONE){
    if(par_config.used[i]){
      temp+=comm_dat.zone_temp[i];
      zone_cnt++;
    }
  }
  return (zone_cnt==0?0:(temp/zone_cnt));
}
...
void task_pid_autotune(uint8_t id,int32_t ntick)
{
   ...
   COMM_BIT_SET(bit_auto_pid);
   cmd_string(CMD_T303,"E-%d C%d S%d",g_m303.z,g_m303.c,(int)g_m303.temp);
   crWait(id,2000);
   if(COMM_BIT_IS0(bit_auto_pid)){
      do{
        COMM_BIT_SET(bit_pid_state);
        cmd_string(CMD_T304,"");
        crWait(id,2000UL);
        if(COMM_BIT_IS1(bit_pid_state)){
...
void task_set_bed_pid(uint8_t id,int32_t ntick)
{
   crBegin(id, ntick);
   thermalManager.disable_all_heaters();
   COMM_BIT_SET(bit_set_pid);
   cmd_string(CMD_T107,"%.2f %.2f %.2f %d",g_m304.p,g_m304.i,g_m304.d, g_m304.z);
   crWait(id,2000);
   if(COMM_BIT_IS0(bit_set_pid))
   {
    #define _SET_PID(XXX) do{par_config.pid##XXX[0]=g_m304.p;par_config.pid##XXX[1]=g_m304.i;par_config.pid##XXX[2]=g_m304.d;}while(0)
    SERIAL_ECHOLNPGM(" p:", g_m304.p,
                  " i:", g_m304.i,
                  " d:", g_m304.d,
                  " z:", g_m304.z);
    ...
    if(g_m304.z==-1)
    {
      _SET_PID(1);_SET_PID(2);_SET_PID(3);_SET_PID(4);
      _SET_PID(5);_SET_PID(6);_SET_PID(7);_SET_PID(8);
      _SET_PID(9);     
    }else
    {
      #define _IF_SET_PID(XXX) if(g_m304.z==XXX)_SET_PID(XXX)
      _IF_SET_PID(1);_IF_SET_PID(2);_IF_SET_PID(3);_IF_SET_PID(4);
      _IF_SET_PID(5);_IF_SET_PID(6);_IF_SET_PID(7);_IF_SET_PID(8);
      _IF_SET_PID(9);
    }
  }else
  {
    SERIAL_ECHOLNPGM_P("comm err");
  }
  crDel(id);
  crFinish(id)
}

void task_start_heating(uint8_t id,int32_t ntick)
{
  crBegin(id, ntick);
  COMM_BIT_SET(bit_start_heating);
  cmd_string(CMD_T102,"%d",(int)g_m140_m190.target_temp);
  crWait(id,1000);
  if(COMM_BIT_IS1(bit_start_heating)){
    thermalManager.bedheat_info("zone module comm err");
      crDel(id);
      return;
  }

  thermalManager.setTargetBed(g_m140_m190.target_temp);
  thermalManager.isHeatingBed() ? LCD_MESSAGE(MSG_BED_HEATING) : LCD_MESSAGE(MSG_BED_COOLING);
  TERN_(PRINTJOB_TIMER_AUTOSTART, thermalManager.auto_job_check_timer(g_m140_m190.is_m190, !g_m140_m190.is_m190));

  if (g_m140_m190.is_m190){
    crLock(id);
    zooe_state_reset();
    thermalManager.wait_for_bed(g_m140_m190.no_wait_for_cooling);
    crUnlock(id);
  }else
  {
