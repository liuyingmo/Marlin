...
void Temperature::max_temp_error(const heater_id_t heater_id,int zone_id) {
  char str[32]="";
  sprintf(str,"%s %d",STR_T_MAXTEMP,zone_id); 
  _temp_error(heater_id,str, GET_TEXT(MSG_ERR_MAXTEMP));
}

void Temperature::min_temp_error(const heater_id_t heater_id,int zone_id) {
  char str[32]="";
  sprintf(str,"%s %d",STR_T_MINTEMP,zone_id); 
  _temp_error(heater_id,str,GET_TEXT(MSG_ERR_MINTEMP));
}
