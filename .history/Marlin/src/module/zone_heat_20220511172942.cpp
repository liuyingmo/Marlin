...
void sw_uart_recv(sw_uart_rece_buf_t *p_sws,char c)
{
   uint16_t msg_pos =0;
   ...
   if(c =='\r' || c =='\n') p_sws->end_flag++;
        ...
        p_sws->end_flag =0;
        if(p_sws->index<SWS_BUF_SIZE){
          p_sws->buf[p_sws->index++] =c;
        }
   }
   ...
   if(p_sws->end_flag>0)
   {
     if(p_sws->index){
        ...
        if(msg_pos!=0xffff){
					memcpy(msg_bed_buf[msg_pos].buf,p_sws->buf,SWS_BUF_SIZE);
          msg_bed_buf[msg_pos].used =SWS_USE_EXI;
        }
     }
		p_sws->end_flag =0;
		p_sws->index=0;
    memset(p_sws->buf,0,SWS_BUF_SIZE);
   }
}
