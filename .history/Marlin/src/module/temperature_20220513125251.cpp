...
void Temperature::max_temp_error(const heater_id_t e,int zone_id) {
  ...
  _temp_error(e,(FSTR_P const)str, GET_TEXT(MSG_ERR_MAXTEMP));
  _temp_error(e, F(STR_T_MAXTEMP), GET_TEXT_F(MSG_ERR_MAXTEMP));
}

void Temperature::min_temp_error(const heater_id_t e,int zone_id) {
  char str[32]="";
  sprintf(str,"%s %d",STR_T_MINTEMP,zone_id); 
  _temp_error(e,(FSTR_P const)str,GET_TEXT(MSG_ERR_MINTEMP));
  _temp_error(e, F(STR_T_MINTEMP), GET_TEXT_F(MSG_ERR_MINTEMP));
}
