...
void msg_bed_deal(void)
{
  LOOP_L_N(i,MAX_SMS){
    if(msg_bed_buf[i].used!=SWS_USE_EMP){
      to_uper(msg_bed_buf[i].buf);
      LOOP_L_N(j,GetItemSum(const_msg_cmd)){
         if (const_msg_cmd[j].cmd == NULL || const_msg_cmd[j].func==NULL) continue;
         if (check_msg_cmd(msg_bed_buf[i].buf,const_msg_cmd[j].cmd)){
            int len =strlen(const_msg_cmd[j].cmd);
            const_msg_cmd[j].func(&msg_bed_buf[i].buf[len]);
         }
      }
      msg_bed_buf[i].used =SWS_USE_EMP;
    }
  }
}
