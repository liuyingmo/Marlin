...
void task_set_bed_pid(uint8_t id,int32_t ntick)
{
   ...
   bed_string("%s %.2f %.2f %.2f %d\n",CMD_T107,g_m304.p,g_m304.i,g_m304.d, g_m304.u);
   crWait(id,2000);
   if(!(comm_bit_updata&MaskBits[bit_set_pid]))
   {
   #define _SET_PID(XXX) do{par_config.pid##XXX[0]=g_m304.p;par_config.pid##XXX[1]=g_m304.i;par_config.pid##XXX[2]=g_m304.d;}while(0)

    SERIAL_ECHO_MSG(" p:", g_m304.p,
                  " i:", g_m304.i,
                  " d:", g_m304.d,
                  " u:", g_m304.u);  
    if(g_m304.u==-1)
    ...
    {
      #define _IF_SET_PID(XXX) if(g_m304.u==XXX)_SET_PID(XXX)
