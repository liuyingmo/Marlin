...
#define GET_PARAM_PID(XXX) {\
  ...
  debug_string("%.2f,%.2f,%.2f\n",par_temp.pid##XXX[0],par_temp.pid##XXX[1],par_temp.pid##XXX[2]);\
  param_bit_updata|=MaskBits[XXX];\
}
...
#define GET_PARAM_STEP(XXX){\
  ...
  debug_string("%d,%d\n",par_temp.keepstep,par_temp.slowstep);\
  param_bit_updata|=MaskBits[XXX];\
}
...
#define GET_PARAM_USED(XXX){\
  ...
  debug_string("%d,%d,%d,%d,%d,%d,%d,%d,%d\n",par_temp.used[0],par_temp.used[1],par_temp.used[2],par_temp.used[3],\
...
#define GET_TEMPLIMIT(XXX){\
  ...
  param_bit_updata|=MaskBits[XXX];\
  debug_string("%d,%d\n",par_temp.max_temp,par_temp.max_temp);\
}
...
void task_start_up(uint8_t id,int32_t ntick)
{
  ...
  if(0 && par_config.compile_time!=const_par.compile_time){
    do{
    debug_string("write flash\n");
    COMM_BIT_SET(bit_flash_init);
    cmd_string(CMD_T105,"0 %d",const_par.compile_time);
    crWait(id,10);
    cmd_string(CMD_T105,"1 %.2f %.2f %.2f",const_par.pid1[0],const_par.pid1[1],const_par.pid1[2]);
    crWait(id,10);
    cmd_string(CMD_T105,"2 %.2f %.2f %.2f",const_par.pid2[0],const_par.pid2[1],const_par.pid2[2]);
    crWait(id,10);
    cmd_string(CMD_T105,"3 %.2f %.2f %.2f",const_par.pid3[0],const_par.pid3[1],const_par.pid3[2]);
    crWait(id,10);
    cmd_string(CMD_T105,"4 %.2f %.2f %.2f",const_par.pid4[0],const_par.pid4[1],const_par.pid4[2]);
    crWait(id,10);
    cmd_string(CMD_T105,"5 %.2f %.2f %.2f",const_par.pid5[0],const_par.pid5[1],const_par.pid5[2]);
    crWait(id,10);
    cmd_string(CMD_T105,"6 %.2f %.2f %.2f",const_par.pid6[0],const_par.pid6[1],const_par.pid6[2]);
    crWait(id,10);
    cmd_string(CMD_T105,"7 %.2f %.2f %.2f",const_par.pid7[0],const_par.pid7[1],const_par.pid7[2]);
    crWait(id,10);
    cmd_string(CMD_T105,"8 %.2f %.2f %.2f",const_par.pid8[0],const_par.pid8[1],const_par.pid8[2]);
    crWait(id,10);
    cmd_string(CMD_T105,"9 %.2f %.2f %.2f",const_par.pid9[0],const_par.pid9[1],const_par.pid9[2]);
    crWait(id,10);
