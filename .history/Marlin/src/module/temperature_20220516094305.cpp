...
void Temperature::max_temp_error(const heater_id_t e,int zone_id) {
  char str[64]="";
  sprintf(str,"%s %d",STR_T_MAXTEMP,zone_id); 
  _temp_error(e, (FSTR_P const)str, GET_TEXT_F(MSG_ERR_MAXTEMP));
}

void Temperature::min_temp_error(const heater_id_t e,int zone_id) {
  char str[64]="";
  sprintf(str,"%s %d",STR_T_MINTEMP,zone_id); 
  _temp_error(e, (FSTR_P const)str, GET_TEXT_F(MSG_ERR_MINTEMP));
}

void Temperature::bedheat_timeout_error(uint8_t zone_id) {
  char str[64]="";
  sprintf(str,"%s %d",str_t_heating_failed,zone_id);
  _temp_error(H_BED,(FSTR_P const)str, GET_TEXT_F(MSG_HEATING_FAILED_LCD));
}

void Temperature::bedheat_info(const char *str)
{
  _temp_error(H_BED,(FSTR_P const)str, GET_TEXT(MSG_HEATING_FAILED_LCD));
}
...
void Temperature::updateTemperaturesFromRawValues() {
  ...
  #if BED_ZONE_HEAT
    TERN_(HAS_HEATED_BED,     temp_bed.celsius       = get_heat_temp());
  #else
